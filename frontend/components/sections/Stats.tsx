  ...
  return (
    <section className="border-y border-dourado/20 bg-preto py-16">
      <div className="container-page grid gap-8 md:grid-cols-4">
        {stats.map((stat) => (
          <div key={stat.label} className="border-dourado/30 md:border-r md:last:border-r-0">
            <div className="font-serif text-5xl text-dourado md:text-6xl">
              <AnimatedCounter value={stat.value} suffix={stat.suffix} />
            </div>
            <p className="mt-3 text-sm uppercase tracking-[0.18em] text-cinza">{stat.label}</p>
          </div>
        ))}
      </div>
    </section>
